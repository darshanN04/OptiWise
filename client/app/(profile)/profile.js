...
const Profile = () => {
    ...
    const fetchDoctorProfile = async () => {
      try {
        ...
        if (token && storedEmail) {
          ...
          const response = await fetch(`http://192.168.165.145:${PORT}/v1/doctor/profile?email=${storedEmail}`, {
            method: 'GET',
            headers: {
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch(`http://192.168.165.145:${PORT}/v1/doctor/logout`, {
        method: 'POST',
        headers: {
